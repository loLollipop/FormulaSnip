...
智能模式
  ...
  └─ 困难公式：调用默认安装的 PP-FormulaNet-S 复核
         ├─ 质量检查选择较干净的候选
         └─ 两个结果都保留，允许用户切换
```

当前产品决策改为默认同时安装 Rapid 与 Paddle，使智能复核和精确模式开箱可用。本机将两者装入同一环境后，依赖目录与两个模型合计约 1.47 GiB；这是以约 785 MiB 的额外体积换取完整识别路径。智能模式仍只在风险条件命中时运行 Paddle，并非每次都运行两个模型。
...
2. **精确模式**：直接使用默认安装的 PP-FormulaNet-S。
...
- 验证默认 Paddle 依赖在打包后的独立子进程中运行，避免 DLL/内存影响主界面；
